...
class MovieRetrieverTool(BaseTool):
    ...
    description: str = """Tool for searching movie using both SQL and semantic search."""
    ...
    def _run(self, sql_query: str = None, semantic_query: str = None, run_manager: Optional[CallbackManagerForToolRun] = None):
        ...
        context = json.dumps([item["metadata"] for item in items], ensure_ascii=False, indent=4)
        return context

    async def _arun(
        self,
        sql_query: str = None, semantic_query: str = None,
...
def create_vector_db(contexts_df=None, persist_directory="./chroma_db", batch_size=40000):
   """
   ...
   for start_idx in range(0, total_rows, batch_size):
       ...
       print(f"Processing batch {start_idx//batch_size + 1} of {(total_rows + batch_size - 1)//batch_size}")
       
       for _, row in batch_df.iterrows():
           # 메타데이터 생성
           metadata = row.to_dict()
           metadata = {k: str(v) if isinstance(v, (int, float)) else v 
                      for k, v in metadata.items()}
           
           # Document 객체 생성
           doc = Document(
               page_content=str(row['Synopsis']),
               metadata=metadata
           )
